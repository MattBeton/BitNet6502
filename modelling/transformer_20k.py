...
class FeedForward(nn.Module):
    def __init__(self, n_embd: int, ff_mult: int, ff_hidden_dim: int | None, dropout: float) -> None:
        super().__init__()
        hidden_dim = ff_hidden_dim if ff_hidden_dim is not None else ff_mult * n_embd
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.ff = FeedForward(n_embd, ff_mult, ff_hidden_dim, dropout)
...
class CausalTransformerLM(nn.Module):
    def __init__(self, vocab_size: int, cfg: Config) -> None:
        ...
        positions = torch.arange(cfg.block_size, dtype=torch.float32).unsqueeze(1)
        div_term = torch.exp(
            torch.arange(0, cfg.n_embd, 2, dtype=torch.float32) * (-math.log(10000.0) / cfg.n_embd)
        )
        position_embedding = torch.zeros(cfg.block_size, cfg.n_embd)
        position_embedding[:, 0::2] = torch.sin(positions * div_term)
        position_embedding[:, 1::2] = torch.cos(positions * div_term[: position_embedding[:, 1::2].shape[1]])
    ...
    def forward(self, idx: torch.Tensor, targets: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor | None]:
        ...
        x = self.token_embedding(idx) + self.position_embedding[:seq_len]
        x = self.dropout(x)

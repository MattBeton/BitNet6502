...
class DiagonalSSMLayer(nn.Module):
    """S4D-style diagonal SSM block."""

    def __init__(self, n_embd: int, state_size: int, conv_kernel: int, dropout: float) -> None:
        super().__init__()
        self.n_embd = n_embd
...
class StateSpaceLM(nn.Module):
    def __init__(self, vocab_size: int, cfg: Config) -> None:
        ...
        self.blocks = nn.ModuleList(
            [DiagonalSSMLayer(cfg.n_embd, cfg.state_size, cfg.conv_kernel, cfg.dropout) for _ in range(cfg.n_layer)]
        )

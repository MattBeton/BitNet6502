...
/* Softmax sample: probability-weighted pick from the top-k logits using a
 * 16-byte LUT approximation of `exp(-d / T)`. The caller provides the LUT
 * (see `exp_lut` in weights.c, generated by export_weights.py with the
 * sampling temperature baked in). 16-bit r is drawn via two LCG advances
 * (low byte first); modulo by `sum` is via while-subtract.
 *
 * Mutates `logits` (mask with INT_MIN per pick) — same convention as
 * top_k_sample. Byte-exact match with `softmax_sample` in Python.
 */
